...
class HabitDetailViewController: UIViewController {
    ...
    func updateCollectionView(){
        ...
        dataSource.applySnapshotUsing(sectionIDs: [.remaining], itemBySections: [.remaining: items.map(\.id)])
    }
    ...
    func createDataSource() -> DataSourceType{
        let cellRegistration = UICollectionView.CellRegistration<UICollectionViewListCell, ViewModel.Item.ID> {[weak self] cell, indexPath, itemIdentifier in
            guard let self, let grouping = items.first(where: {$0.id == itemIdentifier}) else {return}
